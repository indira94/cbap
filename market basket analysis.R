...
inspect(subset1)
inspect(rules2)
subset2 = subset(rules2, subset=lhs %ain% c('baking powder','soda') ) #ain => both together
inspect(subset2)
subset2a = subset(rules2, subset=lhs %in% c('baking powder','soda') ) #in => either
inspect(subset2a)
...
plot(subset2a) 
plot(subset2a, measure=c("support", "lift"), shading="confidence")
